...
class ReductoRequest extends TransportMessage {
	...
	def this(request:String, rt:String) {
	  ...
	  text = if (!reqJson.path("text").isMissingNode()) clean(reqJson.path("text").asText()) else null
	...
	def this(request:HttpRequest, rt:String) {
	  ...
	  text = if (request.uri.query.get("text") != None) clean(request.uri.query.get("text").get) else null
	...
	def this(headlineString:String, textString:String, rt:String){
	  ...
	  text = clean(textString)
	  request_type = rt
	  
	}
	
	
	// Remove easy formatting issues
	def clean(dirty:String): String = {
		if (dirty == null || dirty.length() == 0) {
			return dirty
		}
		
		val clean = dirty.replaceAll("""[A-Z]*—""", "")
		clean
	}
